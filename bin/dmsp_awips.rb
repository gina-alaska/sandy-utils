...
class DMSPAwipsClamp <  ProcessingFramework::CommandLineHelper
  ...
  def check_input(infile)
	...
	raise("infile #{infile} isn't a ols file, and isn't a directory") if !File.directory?(infile)
	raise("infile #{infile} isn't a ols file, and isn't a directory containing a ols file.") if infile.length == 0
